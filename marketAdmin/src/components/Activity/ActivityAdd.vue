<template>
  <div class="activityadd">
    ...
    <div class="content">
      <ul class="addactivitycontent">
        ...
        <li>
          ...
          <div class="addcontent">
            <Radio-group v-model="activity.skin" type="button" class="skinradio">
              <Radio label="2" style="padding:0px;margin-right:10px">
                <img src="/static/images/skin/skin1.png" alt="" style="height:100%;margin:0px">
              </Radio>
              <Radio label="3" style="padding:0px;margin-right:10px">
                <img src="/static/images/skin/skin2.png" alt="" style="height:100%;margin:0px">
              </Radio>
  
              <Radio label="4" style="padding:0px;margin-right:10px">
                <img src="/static/images/skin/skin3.png" alt="" style="height:100%;margin:0px">
              </Radio>
  
              <Radio label="5" style="padding:0px;margin-right:10px">
                <img src="/static/images/skin/skin4.png" alt="" style="height:100%;margin:0px">
              </Radio>
        ...
        <li>
          ...
          <div class="addcontent" @click="getScroll('uploader')">
        ...
        <li>
          ...
          <div class="addcontent" @click='musicSelect'>
            <!-- <div>{{musicName}}</div> -->
            <Input v-model="musicName"></Input>
        ...
        <li v-if="activity.shareGift == 1">
          ...
          <div class="addcontent" style="margin-top:15px;">
            <ul class="giftlist">
              <li v-for="(gl,index) in GiftList" @click="setGift(gl)">
                <img :src="'/static/images/'+ (gl.selected?'':'un')+ 'selected.png'" alt="">
                <span v-text="gl.giftName"></span>
              </li>
            </ul>
          </div>
          <div class="addnote">
        ...
        <li>
          ...
          <div class="addcontent" style="margin-top:15px;">
            <ul class="giftlist">
              <li v-for="(gl,index) in goodList" @click="setGift(gl)">
                <img :src="'/static/images/'+ (gl.selected?'':'un')+ 'selected.png'" alt="">&nbsp&nbsp
                <span v-text="gl.goodsName"></span>
              </li>
            </ul>
          </div>
          <div class="addnote">
        ...
        <li>
          ...
          <div class="addcontent ueditor-wrapper">
            <ueditor @click="getScroll('editor')" :value="defaultMSg" :config="Ueditorconfig" @input="Ueditorinput" v-on:ready="Ueditorready"></ueditor>
          </div>
        </li>
        <li>
    ...
    <div style="position:fixed;right:20px;top:50%;width:100px;height:100px;" v-if='!previewShow'>
      <Button type="info" @click="previewShow=true">即时预览</Button>
    </div>
    <div class="closeTitle" v-if="previewShow">
      <div class="close" @click='previewShow=false'></div>
    </div>
    <Preview class='preview' :activity='activity' :anchor='anchor' v-if="previewShow"></Preview>
    <Modal v-model="musicModal" title="活动背景音乐">
      <Table stripe :columns="musciCol" :data="musicList" @on-row-click='getMusicId'></Table>
      <div style='height:40px;padding-top:10px'>
        <Page styles="float:right" :total="musicTotalPages" :current="musiccurrPage" :size='12'></Page>
      </div>
      <div slot="footer"></div>
    </Modal>
  </div>
...
export default {
  components: { ueditor, uploader , Preview},
  name: 'ActivityAdd',
  mounted() {
    this.over = true;
  },
    ...
    getMusicList(pageNo) {
      this.http.get(this.$store.state.prefix + '/music/page/' + (pageNo || 1)).then(res => {
        if (res.error === false) {
          ...
          this.musiccurrPage = res.result.current
          this.musicTotalPages = res.result.pages
        }
      })
    },
    saveActivity() {
