...
export default {
	...
	methods: {
		getWithDrawList(pageno) {
			...
			this.http.get(this.$store.state.prefix + "/operate/getCompanyWithdrawInfo/" + param.current + "?a=1" + this.util.parseParam(param)).then(res => {
				...
				if (res.error == false) {
					...
					res.result.records.forEach(item => {
						item.remarks = item.remarks.substring(0, 10)
						item.dealDate = item.dealDate.substring(0, 5)
					})
		...
		applyWithdraw(row) {
			...
			this.$Modal.confirm({
				title: '确认划款',
				content: '<p>确认为' + (row.realName || row.nickName) + '划款：' + Math.abs(row.withdrawAmount) + '元</p>',
				onOk: () => {
